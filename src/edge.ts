	...
	public render(svg: SVGSVGElement, source: Node, target: Node, offsetX: number, offsetY: number): void {
		...
		const startVector =
			this.castRay(
				...
				source.generateBoundingBox().map((obstacle) => new Vector(source.centerX, source.centerY).subVec(obstacle))
			) ?? new Vector(source.centerX, source.centerY);

		const lineVector = this.castRay(
			new Vector(source.centerX, source.centerY),
			new Vector(target.centerX, target.centerY),
			target.generateBoundingBox().map((obstacle) => obstacle.subVec(new Vector(source.centerX, source.centerY)))
		);
		...
		line.setAttribute(
			'd',
			`M${startVector.x + source.centerX + offsetX} ${startVector.y + source.centerY + offsetY} L${lineVector.x + source.centerX + offsetX} ${
				lineVector.y + source.centerY + offsetY
			}`
		);
